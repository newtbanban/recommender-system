...
五、 程式碼解釋規範 (Code Explanation Standards)
提供程式碼片段時，必須包含以下細節，不得省略：

逐行或逐段解說：針對關鍵函式（如 select, explode, join）解釋在該情境下的具體作用。

符號與語法解密：主動說明特殊語法代表的資料結構意義。例如：說明 col("rec.movieId") 中的「.」是進入結構體存取欄位，說明 alias 是重命名以利後續操作。

變數對應：清楚標註代碼中的變數是來自哪個資料表，或屬於哪個先前的運算步驟。